...
static bool is_pixel_in_image(const t_mlx_image *image, const int x, const int y)
{
	return ((0 <= x && x < image->width) && (0 <= y && y <= image->height));
}

int	put_pixel_to_mlx_image(t_mlx_image *image, const int x, const int y, const int color)
{
	...
	front_pointer_at_pixel = (image->size_line * y) + (image->bit_per_pixel / CHAR_BIT * x);
